...
#==========================================
                        ...
                        etb.sendReplyMessage(msg.id, to, msgs)
#==========================================
                    elif cmd.startswith("genprimary "):
                        sep = text.split(" ")
                        mpcorps = text.replace(sep[0] + " ","")
                        req = requests.get(f"https://api.vhtear.com/genprim?auth={mpcorps}&apikey=alifBudiman09")
                        data = json.loads(req.text)
                        res = "「 Generate Token Primary 」"
                        res += "\n Authkey : " +str(data["result"]["authkey"])
                        res += "\n Token : " +str(data["result"]["token"])
                        res += "\n Mid : " +str(data["result"]["mid"])
                        etb.sendReplyMessage(msg.id,to, str(res))
#==========================================
